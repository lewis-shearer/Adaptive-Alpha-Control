...
Identical to ../dynamic.py, adapted for the German Credit dataset instead of
...
Architecture:
  - Predictor P: features -> credit-good/bad prediction
...
NOTE: ALPHA_INIT / ALPHA_LR / W_DEO / W_DAO / W_ACC / EMA_ALPHA below are the
same values found via hyperparameter tuning on the Adult dataset in
../dynamic.py -- kept unchanged here so this is a like-for-like ablation of
"same controller, different dataset" rather than "re-tuned controller". If
the controller behaves poorly on German Credit, that's itself a useful data
point -- it would suggest these hyperparameters don't transfer and a fresh
W&B sweep is needed for this dataset.
...
Run from the repo root: `python german/dynamic.py`
"""
...
# constants found through hyperparam tuning with wandb sweeps (on Adult -- see note above)
...
# target = classification (1 = good credit, 0 = bad credit)
...
for SEED in SEEDS:
    ...
    wandb.init(
        ...
        name=f"dynamic_german_gender_seed{SEED}",
        config={
            "epochs":      EPOCHS,

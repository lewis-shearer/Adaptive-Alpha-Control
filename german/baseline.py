...
Identical to ../baseline.py, adapted for the German Credit dataset instead
of the Adult Income dataset:
...
Kept structurally IDENTICAL to fixed.py / dynamic.py in this folder (same
preprocessing, same build_predictor, same compute_metrics, same per-seed
model rebuild, same seed sweep) -- the only difference is that there is no
adversary and no alpha term in the loss, so this is a valid ablation rather
...
Run from the repo root: `python german/baseline.py`
"""
...
for SEED in SEEDS:
    ...
    wandb.init(
        ...
        name=f"baseline_german_gender_seed{SEED}",
        config={
            "epochs":     EPOCHS,
